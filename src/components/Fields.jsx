export function Field({ field, value, onChange, error }) {
  ...
  const common={id,name:field.name,value:value??'',required:true,onChange:e=>onChange(field.name,e.target.value),'aria-invalid':!!error,'aria-describedby':error?`${id}-error`:undefined,className:'field-control'}
