...
export default function Layout(){
  ...
  const [dark,setDark]=useState(()=>{const saved=localStorage.getItem('smartcalc-theme');return saved?saved==='dark':matchMedia('(prefers-color-scheme: dark)').matches})
  const toggleTheme=()=>{const next=!dark;setDark(next);document.documentElement.classList.toggle('dark',next);localStorage.setItem('smartcalc-theme',next?'dark':'light')}
