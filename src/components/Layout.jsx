...
export default function Layout(){
  ...
  return <div className="app-shell">
    ...
    <main><Outlet/></main>
    <footer><div className="footer-grid"><div className="footer-brand"><Brand/><p>Accurate, straightforward calculators for everyday decisions.</p></div><div><h2>Categories</h2><nav>{Object.entries(categories).slice(0,3).map(([key,c])=><Link key={key} to={`/calculators?category=${key}`}>{c.name}</Link>)}</nav></div><div><h2>More tools</h2><nav>{Object.entries(categories).slice(3).map(([key,c])=><Link key={key} to={`/calculators?category=${key}`}>{c.name}</Link>)}</nav></div><div><h2>Resources</h2><nav><Link to="/calculators">All Calculators</Link><Link to="/about">About & Methodology</Link><Link to="/privacy">Privacy</Link></nav></div></div><div className="footer-bottom"><p>© {new Date().getFullYear()} SmartCalc. All rights reserved.</p><p>Results are informational estimates.</p></div></footer>
  </div>
}
