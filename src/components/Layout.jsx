...
export default function Layout(){
  ...
  return <div className="app-shell">
    ...
    {open&&<div className="drawer-layer" role="presentation" onMouseDown={()=>setOpen(false)}><aside id="mobile-navigation" className="drawer" role="dialog" aria-modal="true" aria-label="Mobile navigation" onMouseDown={event=>event.stopPropagation()}><div className="drawer-head"><Brand/><button className="icon-button" onClick={()=>setOpen(false)} aria-label="Close menu"><X size={20}/></button></div><nav onClick={()=>setOpen(false)}><Link className={isDirectoryActive?'active':''} to="/calculators"><span>00</span>All calculators</Link>{Object.entries(categories).map(([key,category],index)=><Link className={isCategoryActive(key)?'active':''} key={key} to={`/calculators?category=${key}`}><span>0{index+1}</span>{category.name}</Link>)}</nav></aside></div>}
    <main><Outlet/></main>
    <footer><div className="footer-grid"><div className="footer-brand"><Brand/><p>Focused calculators with documented methods, careful validation, and practical results.</p></div><div><h2>Categories</h2><nav>{Object.entries(categories).slice(0,3).map(([key,category])=><Link key={key} to={`/calculators?category=${key}`}>{category.name}</Link>)}</nav></div><div><h2>More tools</h2><nav>{Object.entries(categories).slice(3).map(([key,category])=><Link key={key} to={`/calculators?category=${key}`}>{category.name}</Link>)}</nav></div><div><h2>Resources</h2><nav><Link to="/calculators">All Calculators</Link><Link to="/about">About & Methodology</Link><Link to="/privacy">Privacy</Link></nav></div></div><div className="footer-bottom"><p>© {new Date().getFullYear()} SmartCalc. All rights reserved.</p><p>Results are informational estimates.</p></div></footer>
  </div>
}
