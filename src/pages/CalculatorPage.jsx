...
import { methodologySources } from '../data/methodology'
...
function display(value,suffix,values){if(value===null)return'Not available';if(typeof suffix==='function')return suffix(value,values);if(Array.isArray(value))return `${value[0]}–${value[1]}${suffix||''}`;return `${value}${suffix||''}`}
...
export default function CalculatorPage(){
  ...
  const submit=e=>{e.preventDefault();try{const missing=tool.fields.find(field=>String(values[field.name]??'').trim()==='');if(missing)throw Error(`${missing.label} is required.`);const output=tool.calculate(values);if(!output||Object.values(output).some(x=>typeof x==='number'&&!Number.isFinite(x)))throw Error('The result could not be calculated.');setResult(output);setError('')}catch(err){setResult(null);setError(err.message||'Check your inputs and try again.')}}
  ...
  return <div className="page-container calculator-page">
    ...
    <section className="content-grid"><article><p className="kicker">How this is calculated</p><h2>Method and assumptions</h2><p>{tool.description}</p><h3>Formula</h3><p>{tool.formula}</p><h3>Example</h3><p>{tool.example}</p>{methodologySources[tool.id]&&<><h3>Methodology references</h3><ul className="methodology-links">{methodologySources[tool.id].map(source=><li key={source.url}><a href={source.url} target="_blank" rel="noreferrer">{source.label}</a></li>)}</ul></>}</article><aside><h2>How to use it</h2><ol><li>Enter each requested value and choose the matching units.</li><li>Select <strong>Calculate</strong> to validate the inputs.</li><li>Review the primary result, supporting values, and assumptions.</li></ol></aside></section>
